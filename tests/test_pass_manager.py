...
class TestPasswordManager(unittest.TestCase):
    ...
    def test_create_key(self):
        user_name = "test_user"
        result = self.password_manager.create_key(
            self.key_path, self.test_password, user_name
        )
    ...
    def test_load_password_file(self):
        ...
        site_encrypted = Fernet(self.password_manager.key).encrypt(
            self.test_site.encode()
        )
        pass_encrypted = Fernet(self.password_manager.key).encrypt(
    ...
    def test_add_password(self):
        ...
        self.password_manager.add_password(
            self.test_site, self.test_password, self.key_path, self.site_path
        )
        ...
        with open(self.site_path, "r") as f:
            for line in f:
                ...
                password = (
                    Fernet(self.password_manager.key)
                    .decrypt(password.encode())
                    .decode()
                )
    ...
    def test_get_pass(self, mock_load_password_file):
        ...
        with patch("sqlite3.connect") as mock_connect:
            self.password_manager.get_pass(
                self.key_path, self.pass_path, self.test_site
            )
    ...
    def test_get_encrypted(self):
        ...
        encrypted_login, encrypted_password = PasswordManager.get_encrypted(
            self.key_path
        )
    ...
    def test_encrypt_password(self):
        ...
        with patch("hashlib.pbkdf2_hmac") as mock_hash:
            ...
            hashed_login, hashed_password = PasswordManager.encrypt_password(
                self.key_path, "test", "test"
            )
