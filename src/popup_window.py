...
class PopUpWindow:
    ...
    def create_window(self) -> None:
        ...
        self.window.geometry(
            "%dx%d+%d+%d" % (window_width, window_height, x * 1.25, y * 1.4)
        )

        self.message_label = ctk.CTkLabel(
            master=self.window, text="", font=("Arial", 12, "bold")
        )
        self.user_label = ctk.CTkLabel(
            master=self.window, text="User name:", font=("Arial", 12, "normal")
        )
        self.path_label = ctk.CTkLabel(
            master=self.window, text="Path: ", font=("Arial", 12, "normal")
        )
    ...
    def delete_user_window(self) -> None:
        ...
        self.username_entry = ctk.CTkEntry(
            master=self.window, width=170, height=25, border_width=1, corner_radius=7
        )
        self.username_entry.place(x=210, y=20, anchor="center")
        self.username_label = ctk.CTkLabel(
            master=self.window, text="Login:", font=("Arial", 12, "normal")
        )
        ...
        self.user_password_label = ctk.CTkLabel(
            master=self.window, text="Password:", font=("Arial", 12, "normal")
        )
        self.user_password_label.place(x=115, y=48, anchor="e")

        self.confirmation_button = ctk.CTkButton(
            master=self.window, text="Confirm", command=self._user_login
        )
        self.confirmation_button.place(x=210, y=80, anchor="center")

        self.user_password_entry.bind("<Tab>", self._focus_on, add="+")
        self.user_password_entry.bind(
            "<Tab>",
            self.confirmation_button.bind("<Return>", self._user_login),
            add="+",
        )
    ...
    def _user_login(self, event: Any = None) -> None:
        ...
        try:
            ...
            typed_login, typed_password = PasswordManager().encrypt_password(
                key_p, _password, self._login
            )
    ...
    def delete_user_account(self) -> None:
        ...
        self.information_label.configure(
            text="Account have been successfully deleted.", text_color="green"
        )

    def confirm_deleting(self) -> None:
        accept_button = ctk.CTkButton(
            master=self.window, text="Yes", command=self.delete_user_account
        )
        accept_button.place(x=100, y=70, anchor="center")

        decline_button = ctk.CTkButton(
            master=self.window, text="No", command=self.close_window
        )
