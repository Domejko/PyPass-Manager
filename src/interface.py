...
class MainUI(PopUpWindow):
    def __init__(self):
        ...
        try:
            self.my_image = ctk.CTkImage(
                dark_image=Image.open("/usr/local/pypass-manager/src/img/logo3.png"),
                size=(146, 189),
            )
        except FileNotFoundError:
            self.my_image = ctk.CTkImage(
                dark_image=Image.open("src/img/logo3.png"), size=(146, 189)
            )

        self.logo_image = ctk.CTkButton(
            master=self.frame,
            image=self.my_image,
            text="",
            border_width=0,
        ...
        self.file_label = ctk.CTkLabel(
            master=self.frame,
            text="Best protection will provide USB stick dedicated "
        ...
        self.login_entry = ctk.CTkEntry(
            master=self.frame, width=170, height=25, border_width=1, corner_radius=7
        )
        self.login_label = ctk.CTkLabel(master=self.frame, text="Login :")

        self.logout_button = ctk.CTkButton(
            master=self.frame, text="Logout", width=20, command=self.logout
        )
        ...
        self.confirm_button = ctk.CTkButton(
            master=self.frame, text="Confirm", command=self.login
        )

        self.error_label = ctk.CTkLabel(
            master=self.frame,
            text="Incorrect password/login. Try again",
            text_color="red",
        )
        ...
        self.find_password_button = ctk.CTkButton(
            master=self.frame,
            text="Find Password",
            width=170,
            height=50,
            font=("Arial", 17, "bold"),
            command=self.search_password_window,
        )
        ...
        self.add_password_button = ctk.CTkButton(
            ...
            height=50,
            font=("Arial", 17, "bold"),
            command=self.check_password_window,
        )
        self.add_password_button.bind("<Return>", self.enter_add)

        self.go_back_button = ctk.CTkButton(
            master=self.frame, text="Menu", width=20, command=self.menu
        )
        self.back_to_login = ctk.CTkButton(
            master=self.frame, text="Login", width=20, command=self.logout
        )

        self.search_button = ctk.CTkButton(
            master=self.frame, text="Search", command=self.search_password
        )
        self.search_button.bind("<Return>", self.enter_search)

        self.new_user_button = ctk.CTkButton(
            master=self.frame, text="Create New User", command=self.create_user_window
        )

        self.create_user_button = ctk.CTkButton(
            master=self.frame, text="Create User", command=self.create_user
        )

        self.check_pwned_button = ctk.CTkButton(
            master=self.frame, text="Check password", command=self.check_password
        )
        self.check_pwned_button.bind("<Return>", self.enter_check)

        self.save_button = ctk.CTkButton(
            master=self.frame, text="Save password", command=self.save_password_window
        )

        self.browse_button = ctk.CTkButton(
            master=self.frame, width=50, text="Browse", command=self.browse_saving_path
        )

        self.delete_user_button = ctk.CTkButton(
            master=self.frame, text="Delete User", command=self.delete_user
        )
        ...
        self.password_entry.bind(
            "<Tab>", self.confirm_button.bind("<Return>", self.login), add="+"
        )
    ...
    def login_window(self) -> None:
        ...
        self.confirm_button.bind(
            "<Tab>", self.confirm_button.configure(fg_color="#1F538D")
        )
    ...
    def login(self, event: Any = None) -> None:
        ...
        try:
            ...
            typed_login, typed_password = self.pm.encrypt_password(
                key_p, self.password, self.user_login
            )
    ...
    def scan_passwords(self):
        ...
        try:
            ...
            message = (
                f"          This site passwords have been detected in hackers database:\n{breached_sites}\n "
    ...
    def create_user_window(self) -> None:
        ...
        self.new_password_entry.bind(
            "<Tab>", self.create_user_button.bind("<Return>", self.create_user), add="+"
        )
        ...
        try:
            ...
            if process_result is True:
                ...
                self.error_label.configure(
                    text="This login already exist, pick different login."
                )
                self.error_label.place(x=330, y=200, anchor="center")
            elif process_result is PermissionError:
                self.error_label.configure(
                    text="USB that you try to write you key on is write protected."
            ...
            self.error_label.configure(
                text="Given path is incorrect. Please pick an existing file path."
            )
            self.error_label.place(x=330, y=200, anchor="center")
    ...
    def search_password(self) -> None:
        ...
        try:
            ...
            message = (
                "Site you entered is incorrect or does not exist.\n "
    ...
    def check_password_window(self) -> None:
        ...
        self.save_button.configure(
            text="Save password", command=self.save_password_window
        )
        ...
        self.info_label.configure(
            text="Here you can check how secure your password is. \n"
    ...
    def save_password_window(self) -> None:
        ...
        self.info_label.configure(
            text="Duplicates WARNING ! If you use the same site name second time old password "
            "will be overwrite."
        )
